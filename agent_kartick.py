...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_kartick.log"), logging.StreamHandler()])
log = logging.getLogger(__name__)

DEDUP_FILE = Path("logs") / "daily_replied_senders_kartick.json"
MAX_DAILY_SENDS = 450
...
REPLIED_LABEL = "AutoReplied_Kartick"
...
def send_reply(email, role, server):
    smtp_email = os.environ["KARTICK_SMTP_EMAIL"]
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Kartick_MuleSoft.docx"')
    msg.attach(part)
...
def log_sent(email, role):
    csv_path = "logs/sent_log_kartick.csv"
...
def main():
    log.info("=" * 70)
    log.info("AI Email Agent - Kartick Kiran (Lead MuleSoft Developer)")
    ...
    required = ["IMAP_EMAIL", "IMAP_APP_PASSWORD", "KARTICK_SMTP_EMAIL", "KARTICK_SMTP_APP_PASSWORD"]
    ...
    try:
        ...
        smtp_server.login(smtp_email, os.environ["KARTICK_SMTP_APP_PASSWORD"])
    ...
    log.info("SEND account : %s", os.environ.get("KARTICK_SMTP_EMAIL"))
