...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[logging.FileHandler("logs/agent_rahul.log"), logging.StreamHandler()])
...
ROLES = [
    {
        # Rahul's core role - Azure focused Data Engineer
    ...
    {
        # Databricks / Spark / PySpark roles
    ...
    {
        # Senior / Lead Data Engineer with cloud stack
...
def send_reply(email, role, server):
    ...
    part.add_header("Content-Disposition", 'attachment; filename="Resume_Rahul_DataEngineer.docx"')
    msg.attach(part)
